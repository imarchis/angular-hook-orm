...
function JH($injector, $q){
    function JH() {
        ...
        h.needsParsing = function needsParsing(json) {
...
function EntitiesManager(JH) {
    function EntitiesManager() {
        ...
        em.allInTable = function allInTable(table, f = false) {
            let ap = _allPersisted(table);
            if (ap.length > 0 && !f) {
                return JH.promise(ap);
            } else {
                return em.fetchAllInTable(table);
            }
        };
        em.fetchAllInTable = function fetchAllInTable(table) {
            let queID = JH.uuid();
            addQue(queID);
                ...
                return DBA.selectAll(table).then(function (r) {
                    ...
                    r.map(function (ent) {
                        if(JH.hasProp(_p, ent[_e.key])) {
                            ...
                            let isDeleted = JH.hasProp(ent, _e.deleted) && ent[_e.deleted] == true;
                            if(!isDeleted) {
                                picks.push(ent);
                            }
                        }
                    ...
                    clearQue(queID);
                    return em.convert(picks);
                });
            });
        }
        em.find = function find(i) {
            return em.findMany(i);
        };
        ...
        em.convert = function convert(docs) {
            ...
            let bulk = [];
            docs.map(function (d) {
                let isDeleted = JH.hasProp(d, _e.deleted) && d[_e.deleted] == true;
                if(JH.hasProp(_p, d[_e.key]) && !isDeleted) {
                    bulk.push(_p[d[_e.key]]);
                } else if (isDeleted) {
                    _rmMemory(d);
                } else {
                    let ent = em.entity(d, true);
                    if (ent != null) {
                        bulk.push(ent);
                    }
                }
        ...
        var _rmMemory = function _rmMemory(d) {
            if (JH.hasProp(_p, d[_e.key])) {
                delete _p[d[_e.key]];
            }
            if (JH.hasProp(_s, d[_e.key])) {
                delete _s[d[_e.key]];
            }
        }
        var _revive = function _revive(e) {
            if (!JH.hasProp(e, _e.key)) {
                e[_e.key] = e[_e.table] + ':' + JH.uuid();
            }
            e.grab = _grab;
            e.hook = _hook;
            e.unhook = _unhook;
            ...
            e.assign = _assign;
            e.unassign = _unassign;
            e.holdBallast = _ballast;
            e.grabBallast = _grabBallast;
            e.updateBallast = _updateBallast;
        ...
        var _prepare = function _prepare(e) {
            ...
            if (JH.hasProp(e, ballast)) {
                e[ballast] = JSON.stringify(e[ballast]);
            }
            if (JH.hasProp(e, 'ballastLoad')) {
                delete e.ballastLoad;
            }
            return e;
        };
        var _spark = function _spark(e) {
            let rel = _e.relations;
            let hooks = _e.hooks;
            let ballast = _e.ballast;
            if (JH.hasProp(e, rel) && JH.needsParsing(e[rel])) {
                e[rel] = JSON.parse(e[rel]);
            }
            if (JH.hasProp(e, hooks) && JH.needsParsing(e[hooks])) {
                e[hooks] = JSON.parse(e[hooks]);
            }
            if (JH.hasProp(e, ballast) && JH.needsParsing(e[ballast])) {
                e[ballast] = JSON.parse(e[ballast]);
            }
            return e;
        };
        em.entity = function entity(obj, persist = false) {
            ...
            if (keys.length === 0 || !JH.hasProp(obj, _e.table)) {
                return null;
            } else if (JH.hasProp(obj, _e.deleted) && obj[_e.deleted] == true) {
                _rmMemory(obj);
            } else {
                let t = obj[_e.table];
                let entity = JH.acquire(t.sentenceCase());
                keys.map(function (attr) {
                    if ([_e.relations, _e.hooks, _e.ballast].indexOf(attr) !== -1) {
        ...
        var _allPersisted = function _allPersisted(table) {
            let picks = [];
            let keys = Object.keys(_p);
            keys.map(function (k) {
                let x = keys.indexOf(k);
                if (k.indexOf(table + ':') !== -1) {
                    picks.push(_p[k]);
                }
            });
            return picks;
        ...
        var _grab = function _grab(name) {
            ...
            if (gotHook) {
                ...
                if (!noRel && lnk) {
                    return em.findMany(lnk).then(function(r) {
                        r = r.constructor === Array ? r : [r];
                        var po = [];
                        r.map(function(i) {
                            _prepBallast(subject, name, i);
                            po.push(i);
                        });
                        return JH.promise(JH.extractIfOne(po));
                    });
        ...
        var _unhook = function _unhook(name) {
            ...
            if (gotHook) {
                ...
                addQue(queID);
                return que(queID).then(function () {
                    return _clearHook(subject, name, 'drop').then(function () {
        ...
        var _emptyHook = function _emptyHook(name) {
            ...
            if (gotHook) {
                ...
                addQue(queID);
                return que(queID).then(function () {
                    return _clearHook(subject, name, 'empty').then(function () {
        ...
        var _vanish = function _vanish(ent, type, name, id) {
            ...
            if (_single(type)) {
                ...
                let index = ent[rel][type][name].indexOf(id);
                ent[rel][type][name].splice(index, 1);
            }
            _unloadBallast(ent, type, name, id);
            return JH.promise(true);
        };
        ...
        var _clearHook = function _clearHook(ent, name, method) {
            ...
            if (gotHook) {
                ...
                if (_many(type) && method === 'change') {
                    ...
                    return _clearRel(ent, name, action).then(function () {
        ...
        var _assign = function _assign(name, ent){
            ...
            addQue(queID);
            return que(queID).then(function () {
                return _markHook(subject, name, ent).then(function () {
                    clearQue(queID);
                    return JH.promise(true);
                });
            });
        };
        var _unassign = function _unassign(name, ent){
            var subject = this;
            _spark(subject);
            _spark(ent);
            let queID = JH.uuid();
            var hooks = _e.hooks;
            var key = _e.key;
            var map = subject[hooks][name];
            var reverseType = _reverse[type];
            var type = map.type;
            var reverseName = map.reverse;
            var id = ent[key];
            var sid = subject[key];
            addQue(queID);
            return que(queID).then(function () {
                return _vanish(subject, type, name, id).then(function(){
                    if (map.mirror) {
                        return _vanish(ent, reverseType, reverseName, sid).then(function(){
                            clearQue(queID);
                            return JH.promise(true);
                        });
                    } else {
        ...
        var _unloadBallast = function _unloadBallast(ent, type, name, id) {
            let ball = _e.ballast;
            delete ent[ball][type][name][id];
        };
        var _rmBallast = function _rmBallast(ent, type, name) {
            let ball = _e.ballast;
            delete ent[ball][type][name];
            if (JH.empty(ent[ball][type])) {
                delete ent[ball][type];
            }
        };
        var _emptyBallast = function _emptyBallast(ent, type, name) {
            let ball = _e.ballast;
            ent[ball][type][name] = {};
        };
        var _noBallast = function _noBallast(ent, type, name) {
            let ball = _e.ballast;
            let noball = !JH.hasProp(ent, ball);
            if (noball) {
                ent[ball] = {};
            }
            let noballtype = !JH.hasProp(ent[ball], type);
            if (noballtype) {
                ent[ball][type] = {};
            }
            return !JH.hasProp(ent[ball][type], name);
        };
        var _setBallast = function _setBallast(subject, name, ent, ballast) {
            let hooks = _e.hooks;
            let ball = _e.ballast;
            let key = _e.key;
            let map = subject[hooks][name];
            let type = map.type;
            let id = ent[key];
            let noBallast = _noBallast(subject, type, name);
            if (noBallast) {
                subject[ball][type][name] = {};
            }
            subject[ball][type][name][id] = JSON.stringify(ballast);
            return JH.promise(true);
        };
        var _prepBallast = function _prepBallast(subject, name, ent) {
            let hooks = _e.hooks;
            let ball = _e.ballast;
            let key = _e.key;
            let map = subject[hooks][name];
            let type = map.type;
            let id = ent[key];
            if (JH.hasProp(subject[ball][type][name], id)){
                ent.ballastLoad = JSON.parse(subject[ball][type][name][id]);
            }
        };
        var _updateBallast = function _updateBallast(name, ent) {
            var subject = this;
            let queID = JH.uuid();
            addQue(queID);
            return que(queID).then(function () {
                return _setBallast(subject, name, ent, ent.ballastLoad).then(function () {
                    em.persist(ent);
                    clearQue(queID);
                    return JH.promise(true);
                });
        ...
        var _grabBallast = function _grabBallast(name, ent) {
            var subject = this;
            _spark(subject);
            _spark(ent);
            let hooks = _e.hooks;
            ...
            var load = null;
            if (JH.hasProp(subject[ball][type][name], id)){
                load = JSON.parse(subject[ball][type][name][id]);
            }
            return JH.promise(load);
        };
        var _ballast = function _ballast(name, ent, ballast){
            var subject = this;
            let queID = JH.uuid();
            addQue(queID);
            return que(queID).then(function () {
                return _setBallast(subject, name, ent, ballast).then(function () {
                    clearQue(queID);
                    return JH.promise(true);
                });
            });
        ...
        em.remove = function remove(e) {
            ...
            delete _s[e[_e.key]];
            delete _pc[e[_e.key]];
            DBA.findOne(e[_e.key], {latest:true, conflict:true}).then(function (doc) {
                // Get the latest doc from db to prevent conflicts.
                e = doc; // Not sure about it. TBD.
                let queID = JH.uuid();
                addQue(queID);
                que(queID).then(function () {
                    ...
                    clearQue(queID);
                    DBA.remove(doc).then(function (result) {
        ...
        em.queryBuilder = function queryBuilder() {
            if (_c.queryBuilder != undefined) {
                return  JH.acquire(_c.queryBuilder);
            }
            return null;
        };
        ...
        em.flush = function flush() {
            return ready().then(function () {
                ...
                if (!JH.empty(b)) {
                    return JH.promise(
                        DBA.bulk(b).then(function (result) {
                        }).catch(function (err) {
                          console.log(err);
                        })
                    );
        ...
        em.hooker = function (name){
            return DBA.join(name);
        }
    }
